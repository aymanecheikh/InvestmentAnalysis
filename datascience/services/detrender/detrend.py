...
def detrend_strat_scoring(strat_eval_df, condition):
    ...
    strat_eval_df.Score = strat_eval_df.apply(
        lambda row: (
            add_score(row.Score)
            if row.Strategy in qualifiers.values
            else row.Score
        ),
        axis=1,
    )
...
def evaluate_detrending_strategies(data):
    ...
    for strategy in DetrendingStrategy.__subclasses__():
        ...
        strategy_evaluation = {
            ...
            "Autocorrelation": Autocorrelation(detrended_data).get_autocorrelation_score(),
        }
    ...
    winners = strat_eval_df.loc[
            strat_eval_df.Score == strat_eval_df.Score.max(),
            "Strategy"
            ].values
